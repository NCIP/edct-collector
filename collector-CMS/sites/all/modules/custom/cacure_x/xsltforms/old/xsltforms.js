...
var valuesSeparator = "__VALUE_SEPARATOR__";
...
XFBind.prototype.refresh = function(ctx, index) {
	...
	for (var i = 0, len = this.nodes.length; i < len; i++) {
		...
		if (bindid && this.element.id != bindid) {
			XFProcessor.error(el, "xforms-binding-exception", "Two binds affect one node: " + bindid + ", " + this.element.id);
		} else {
...
;
	...
	"http://www.w3.org/2002/xforms event" : new XPathFunction(false, XPathFunction.DEFAULT_NONE, false,
		function(attribute) {
			...
			if (context) {
				return context[attribute] != undefined ? context[attribute] : [];
			} else {
				return [];
			}
		} ),
