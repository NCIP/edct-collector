...
Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301  USA
	-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:ajx="http://www.ajaxforms.net/2006/ajx" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" version="1.0">
  ...
  <xsl:template match="xhtml:html | html">
			...
			<html>
				...
				<head>
					...
					<!--<script src="{$resourcesdir}jquery-1.4.2.min.js" type="text/javascript"><xsl:text/></script>-->
							...
							<xsl:if test="not(preceding::xforms:bind[starts-with(@type,$nstype)])">
								<xsl:variable name="nsuri">
									<xsl:choose>
										...
										<xsl:when test="$nstype = 'hcitT'">http://www.healthcit.com/2010/formbuilder</xsl:when>
							...
							<xsl:if test="not(preceding::*/@xsi:type[starts-with(.,$nstype)])">
								<xsl:variable name="nsuri">
									<xsl:choose>
										...
										<xsl:when test="//@*[starts-with(name(),concat($nstype,':'))]"><xsl:value-of select="namespace-uri(//@*[starts-with(name(),concat($nstype,':'))][1])"/></xsl:when>
										<xsl:when test="$nstype = 'xs' or $nstype = 'xsd'">http://www.w3.org/2001/XMLSchema</xsl:when>
										<xsl:when test="$nstype = 'hcitT'">http://www.healthcit.com/2010/xforms</xsl:when>
										<xsl:otherwise>unknown</xsl:otherwise>
									</xsl:choose>
								</xsl:variable>
								<xsl:text>Schema.registerPrefix('</xsl:text><xsl:value-of select="$nstype"/><xsl:text>', '</xsl:text><xsl:value-of select="$nsuri"/><xsl:text>');
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<!-- YGB <option><xsl:call-template name="genid"/>...Loading...</option>  -->
					<option class="xforms-item">
					   <xsl:attribute name="id"><xsl:choose><xsl:when test="@id"><xsl:value-of select="@id"/></xsl:when><xsl:otherwise>xf-itemset-item-<xsl:value-of select="count(preceding::xforms:itemset|ancestor::xforms:itemset)"/></xsl:otherwise></xsl:choose>
							</xsl:attribute>
							<xsl:apply-templates select="xforms:label">
								<xsl:with-param name="appearance">item</xsl:with-param>
			...
			<xsl:variable name="body">
				<xsl:choose>
					...
					<xsl:otherwise>
						<select>
						    <xsl:attribute name="id"><xsl:choose><xsl:when test="@id"><xsl:value-of select="@id"/></xsl:when><xsl:otherwise>xf-itemset-<xsl:value-of select="count(preceding::xforms:itemset|ancestor::xforms:itemset)"/></xsl:otherwise></xsl:choose>
							</xsl:attribute>
							<xsl:call-template name="style">
							   <xsl:with-param name="class">xforms-itemset</xsl:with-param>
						    </xsl:call-template>
			...
			<xsl:choose>
			    <xsl:when test="$p = 'false'">false</xsl:when>
			    <xsl:when test="$p = 'true'">true</xsl:when>
			...
			<xsl:if test="contains($js,&quot;,new NodeTestName('&quot;)">
				...
				<xsl:if test="string-length(substring-before($js2,',')) != 2">
					...
					<xsl:choose>
						...
						<xsl:when test="$p = 'hcitT'">http://www.healthcit.com/2010/xforms</xsl:when>
			...
			<xsl:apply-templates select="*" mode="script"/>
			<xsl:call-template name="listeners"/>
		</xsl:template><xsl:template match="xforms:item | xforms:itemset[ancestor::xforms:*[1][@appearance='full']] | xforms:itemset[ancestor::xforms:*[1][@appearance='minimal']]" mode="script" priority="1">
			<xsl:variable name="lname" select="local-name()"/>
			...
			<xsl:apply-templates select="*" mode="script"/>
			<xsl:call-template name="listeners"/>
		</xsl:template><xsl:template match="xforms:itemset[ancestor::xforms:*[1][string(@appearance)!='minimal' and string(@appearance)!='full']]" mode="script" priority="1">
